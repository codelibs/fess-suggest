...
public class SuggestUtilTest extends TestCase {
    public void test_parseSolrParams() throws Exception {
        Map<String, String> params = SuggestUtil.parseSolrParams(
            "q=content:%e3%81%82%e3%81%84%e3%81%82%e3%81%84&fq=role:guest");
    ...
    public void test_parseQuery() throws Exception {
        ...
        words = SuggestUtil.parseQuery("(content:hoge AND content:fuga) OR (content:hoge AND content:zzz)",
            "content");
        assertEquals("hoge", words.get(0));
        assertEquals("fuga", words.get(1));
