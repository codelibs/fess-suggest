...
public class SuggestTokenizerFactory extends TokenizerFactory implements ResourceLoaderAware {
        ...
        if (excludePartOfSpeech != null) {
            for (String text : excludePartOfSpeech.split(",")) {
                ...
                if (text.length() > 0) {
                    final String[] values = text.split(":");
                    if (values.length == 2) {
                        termChecker.excludePartOfSpeech(values[0].trim(), values[1].trim());
                    }
                }
            }
        }
        ...
        if (includeCharTerm != null) {
            for (String text : includeCharTerm.split(",")) {
                ...
                if (text.length() > 0) {
                    final String[] values = text.split(":");
                    if (values.length == 2) {
                        termChecker.includeCharTerm(values[0].trim(), values[1].trim());
                    }
                }
            }
        }
    ...
    public Tokenizer create(final AttributeFactory factory, final Reader input, final boolean allTokenMode) {
        return new SuggestTokenizer(input, bufferSize, userDictionary, discardPunctuation, mode, termChecker, maxLength, allTokenMode);
    }
    ...
    public void inform(final ResourceLoader loader) {
        try {
            ...
            if (userDictionaryPath != null) {
                final InputStream stream = loader.openResource(userDictionaryPath);
