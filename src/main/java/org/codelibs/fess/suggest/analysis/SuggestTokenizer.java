...
import org.codelibs.fess.suggest.io.AccessibleStringReader;
...
public class SuggestTokenizer extends Tokenizer {
    ...
    public SuggestTokenizer(final Reader input, final int bufferSize, final UserDictionary userDictionaryPara,
            final boolean discardPunctuationPara, final Mode modePara, final TermChecker termChecker, final int maxLength,
            boolean allTokenMode) {
        super(input);
    ...
    public void initialize() {
        ...
        try {
            stream = new JapaneseTokenizer(rd, userDictionary, discardPunctuation, tokenizerMode); // TODO reuse?
            ...
            while (stream.incrementToken()) {
                ...
                final PartOfSpeechAttribute psAtt = stream.getAttribute(PartOfSpeechAttribute.class);
        ...
        if (offset < termListByKuromojiSize) {
            ...
            if (tmpOffset < termListByKuromojiSize) {
                ...
                for (; tmpOffset < partOfSpeechListSize; tmpOffset++) {
                    ...
                    final String termByKuromoji = termListByKuromoji.get(tmpOffset);
                    if (termChecker.check(partOfSpeechList.get(tmpOffset), termByKuromoji, START)) {
                        buffer.append(termByKuromoji);
                        readingBuf.append(readingList.get(tmpOffset));

                        for (int i = 1; tmpOffset + i < partOfSpeechListSize; i++) {
                            final String termPairByKuromoji = termListByKuromoji.get(tmpOffset + i);
                            if (termChecker.check(partOfSpeechList.get(tmpOffset + i), termPairByKuromoji, MIDDLE)) {
                                // TODO remove string conjunction
                                if (inputStr.indexOf(buffer.toString() + termPairByKuromoji) != -1) {
                                    buffer.append(termPairByKuromoji);
                                    readingBuf.append(readingList.get(tmpOffset + i));
                                    end++;
                                } else {
                                    break;
                                }
                ...
                if (buffer != null && tmpOffset < partOfSpeechListSize && buffer.length() > termListByKuromoji.get(tmpOffset).length()) {
    ...
    public static class TermChecker {
        ...
        public TermChecker() {
            final Map<String, List<String>> startParamMap = new HashMap<String, List<String>>(3);
            ...
            final Map<String, List<String>> middleParamMap = new HashMap<String, List<String>>(3);
        ...
        private void updateParam(final String mode, final String target, final String value) {
        ...
        public boolean check(final String partOfSpeech, final String termByKuromoji, final String mode) {
            final Map<String, List<String>> modeParamMap = paramMap.get(mode);
            final List<String> includePartOfSpeechList = modeParamMap.get(INCLUDE_PART_OF_SPEECH);
            final List<String> excludePartOfSpeechList = modeParamMap.get(EXCLUDE_PART_OF_SPEECH);
            final List<String> includeCharTermList = modeParamMap.get(INCLUDE_CHAR_TERM);
            ...
            for (int i = 0; i < includePartOfSpeechList.size(); i++) {
                if (partOfSpeech.indexOf(includePartOfSpeechList.get(i)) != -1) {
                    ...
                    for (int j = 0; j < excludePartOfSpeechList.size(); j++) {
                        if (partOfSpeech.indexOf(excludePartOfSpeechList.get(j)) != -1) {
                            isNg = true;
                        }
                    }
                    if (!isNg) {
