...
public class SpellChecker {
    ...
    public String buildSpellCheckQuery(final String query, final List<String> targetFields, final List<String> labels,
            final List<String> roles) {
        return buildSpellCheckQuery(query, fuzzyValue, targetFields, labels, roles);
    }

    public String buildSpellCheckQuery(final String query, final float fuzzy, final List<String> targetFields, final List<String> labels,
            final List<String> roles) {
        ...
        if (readingList.isEmpty()) {
            queryBuf.append(SuggestConstants.SuggestFieldNames.READING).append(':').append(q);
        } else {
            queryBuf.append(SuggestConstants.SuggestFieldNames.READING).append(':').append(readingList.get(0));
        }
