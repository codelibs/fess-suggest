...
public class SuggestSolrServer {
    ...
    public void add(final SolrInputDocument doc) throws IOException, SolrServerException {
        solrServer.add(doc);
    }

    public void add(final List<SolrInputDocument> documents) throws IOException, SolrServerException {
    ...
    public void deleteByQuery(final String query) throws IOException, SolrServerException {
        solrServer.deleteByQuery(query);
    }

    public SolrDocumentList select(final String query) throws IOException, SolrServerException {

        final SolrQuery solrQuery = new SolrQuery();
        solrQuery.setQuery(query);
        solrQuery.setFields(new String[] { SuggestConstants.SuggestFieldNames.ID, SuggestConstants.SuggestFieldNames.COUNT,
                SuggestConstants.SuggestFieldNames.LABELS, SuggestConstants.SuggestFieldNames.ROLES,
                SuggestConstants.SuggestFieldNames.FIELD_NAME });
        final QueryResponse queryResponse = solrServer.query(solrQuery, SolrRequest.METHOD.POST);
        return queryResponse.getResults();
    }

    public SolrDocumentList get(final String ids) throws IOException, SolrServerException {
        final SolrQuery solrQuery = new SolrQuery();
