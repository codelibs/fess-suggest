package org.codelibs.fess.suggest.service;
...
import org.codelibs.fess.suggest.SpellChecker;
...
import org.codelibs.fess.suggest.entity.SpellCheckResponse;
import org.codelibs.fess.suggest.entity.SuggestItem;
import org.codelibs.fess.suggest.entity.SuggestResponse;
import org.codelibs.fess.suggest.index.IndexUpdater;
...
public class SuggestService {
    ...
    public SuggestService(final Suggester suggester, final SpellChecker spellChecker, final SuggestSolrServer suggestSolrServer) {
    ...
    public SuggestResponse getSuggestResponse(final String q, final List<String> fieldNames, final List<String> labels,
            final List<String> roleList, final int rows) {

        final String suggestQuery = suggester.buildSuggestQuery(q, fieldNames, labels, roleList);
        ...
        if (StringUtil.isNotBlank(suggestQuery)) {
            ...
            //sort
            solrQuery.addSort(SuggestConstants.SuggestFieldNames.BOOST, SolrQuery.ORDER.desc);
            solrQuery.addSort(SuggestConstants.SuggestFieldNames.COUNT, SolrQuery.ORDER.desc);
        ...
        final SuggestResponse suggestResponse = new SuggestResponse(queryResponse, rows, q);
        suggestResponse.setExecTime(execTime);
        return suggestResponse;
    }

    public SpellCheckResponse getSpellCheckResponse(final String q, final List<String> fieldNames, final List<String> labels,
            final List<String> roleList, final int rows) {
        final String spellCheckQuery = spellChecker.buildSpellCheckQuery(q, fieldNames, labels, roleList);
        ...
        if (StringUtil.isNotBlank(spellCheckQuery)) {
            ...
            solrQuery.setRows(rows);
            //sort
            solrQuery.setSort(SuggestConstants.SuggestFieldNames.COUNT, SolrQuery.ORDER.desc);
        ...
        final long execTime = System.currentTimeMillis() - startTime;
        final SpellCheckResponse spellCheckResponse = new SpellCheckResponse(queryResponse, rows, q);
        spellCheckResponse.setExecTime(execTime);
        return spellCheckResponse;
    }

    public void addElevateWord(final String word, final String reading, final List<String> labels, final List<String> roles,
            final long boost) {
    ...
    public void deleteAllElevateWords() {
        try {
            suggestSolrServer.deleteByQuery(SuggestConstants.SuggestFieldNames.SEGMENT + ":" + SuggestConstants.SEGMENT_ELEVATE);
    ...
    public void deleteBadWords() {
        for (final String badWord : badWords) {
            try {
                suggestSolrServer.deleteByQuery(SuggestConstants.SuggestFieldNames.TEXT + ":*" + ClientUtils.escapeQueryChars(badWord)
                        + "*");
            } catch (final Exception e) {
    ...
    public void addSolrParams(final String solrParams, final int dayForCleanup) {
        final Map<String, String> paramMap = SuggestUtil.parseSolrParams(solrParams);
        ...
        for (final String field : supportedFields) {
            ...
            if (dayForCleanup >= 0) {
                final String dayForCleanupStr = DateUtil.getThreadLocalDateFormat().format(new Date(getExpiredTimestamp(dayForCleanup)));
    ...
    public long getContentDocumentNum() {
        return getDocumentNum("*:* NOT " + SuggestConstants.SuggestFieldNames.SEGMENT + ":" + SuggestConstants.SEGMENT_QUERY + " NOT "
                + SuggestConstants.SuggestFieldNames.SEGMENT + ":" + SuggestConstants.SEGMENT_ELEVATE);
    }
