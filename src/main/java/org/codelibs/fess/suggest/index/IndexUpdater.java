...
import org.codelibs.fess.suggest.enums.RequestType;
...
public class IndexUpdater extends Thread {
    ...
    public void run() {
        ...
        while (running.get()) {
            ...
            case ADD:
                ...
                final List<SolrInputDocument> solrInputDocumentList = new ArrayList<SolrInputDocument>(itemSize);
        ...
        if (itemSize > 0) {
            ...
            if (documentList != null) {
                ...
                for (final SolrDocument doc : documentList) {
                    final Object idObj = doc.getFieldValue(SuggestConstants.SuggestFieldNames.ID);
                    if (idObj == null) {
                        continue;
                    }
                    ...
                    for (; itemCount < itemSize; itemCount++) {
                        ...
                        if (item.getDocumentId().equals(id)) {
                            final Object count = doc.getFieldValue(SuggestConstants.SuggestFieldNames.COUNT);
                            if (count != null) {
                                item.setCount(item.getCount() + Long.parseLong(count.toString()));
                            }
                            final Collection<Object> labels = doc.getFieldValues(SuggestConstants.SuggestFieldNames.LABELS);
                            if (labels != null) {
                                final List<String> itemLabelList = item.getLabels();
                            ...
                            final Collection<Object> roles = doc.getFieldValues(SuggestConstants.SuggestFieldNames.ROLES);
                            if (roles != null) {
                                final List<String> itemRoleList = item.getRoles();
                            ...
                            final Collection<Object> fields = doc.getFieldValues(SuggestConstants.SuggestFieldNames.FIELD_NAME);
                            if (fields != null) {
                                final List<String> fieldNameList = item.getFieldNameList();
                            ...
                            final Object segmentObj = doc.getFieldValue(SuggestConstants.SuggestFieldNames.SEGMENT);
                            if (segmentObj != null && StringUtils.isNotBlank(segmentObj.toString())) {
                                final long docType = Long.parseLong(segmentObj.toString());
                                final long currentDocType = Long.parseLong(item.getSegment());
                                if (currentDocType > docType) {
                                    item.setSegment(String.valueOf(docType));
                                }
                                final Object expiresObj = doc.getFieldValue(SuggestConstants.SuggestFieldNames.EXPIRES);
    ...
    protected void mergeSuggestItem(final SuggestItem item1, final SuggestItem item2) {
