...
public class AlphabetConverter implements SuggestReadingConverter {
    ...
    public List<String> convert(final String text) {
        ...
        final List<String> kanaList = katakanaConverter.convert(text);
        if (kanaList.isEmpty()) {
            list.add(text);
            return list;
        }

        for (final String kana : kanaList) {
            final List<StringBuilder> bufList = new ArrayList<StringBuilder>();
    ...
    private List<StringBuilder> deepCopyBufList(
            final List<StringBuilder> bufList) {
